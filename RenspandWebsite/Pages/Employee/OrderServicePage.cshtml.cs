...
{
    using global::RenspandWebsite.Service;
    ...
    public class OrderServicePageModel : PageModel
    {
        ...
        /// <summary>
        /// Asynkront henter og filtrerer ordrer baseret på søgeterm.
        /// </summary>
        public async Task OnGetAsync(string searchTerm)
        {
            SearchTerm = searchTerm;

            var allOrders = await _orderService.GetAllOrdersAsync(); // Bruger din service korrekt
            ...
            if (!string.IsNullOrEmpty(SearchTerm))
            {
                FilteredOrders = allOrders
                    .Where(o => (o.Buyer?.Name?.Contains(SearchTerm, StringComparison.OrdinalIgnoreCase) ?? false) ||
                                (o.Buyer?.PhoneNumber?.Contains(SearchTerm, StringComparison.OrdinalIgnoreCase) ?? false) ||
                                (o.AddressItems?.Any(a =>
                                    a.Address.Street.Contains(SearchTerm, StringComparison.OrdinalIgnoreCase) ||
                                    a.Address.City.Contains(SearchTerm, StringComparison.OrdinalIgnoreCase) ||
                                    a.Address.ZipCode.Contains(SearchTerm, StringComparison.OrdinalIgnoreCase)
                                ) ?? false))
                    .ToList();
            }
            else
